...
sshpass -p $PASSWORD scp ./$DOCKER_IMAGE.tar $REMOTE_USER@$REMOTE_HOST:$DOCKER_IMAGES_PATH
...
sshpass -p $PASSWORD ssh $REMOTE_USER@$REMOTE_HOST "docker run --restart unless-stopped -p 3333:3333 -d $DOCKER_IMAGE"
